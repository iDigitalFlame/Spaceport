...
# This file is saved/loaded to "/opt/spaceport/etc/smd/constants.json" as a JSON file.
...
DIRECTORY_ASSETS = join(DIRECTORY_BASE, "assets")
...
DIRECTORY_CONFIG = join("/opt", NAME_BASE, "etc", "smd")

SOCKET_BACKLOG = 512
SOCKET_MESSAGE_INTEGER_SIZE = 4
SOCKET = join(DIRECTORY_TEMP, "smd", "%s.sock" % NAME_BASE)

CONFIG_CLIENT = "{home}/.config/smd/config.json"
CONFIG_SERVER = join(DIRECTORY_CONFIG, "config.json")
...
HYDRA_EXEC_TOKENS = join(DIRECTORY_ASSETS, "websockify", "run")
...
CPU_STATE = join(DIRECTORY_CONFIG, "cpu.json")
...
TABLET_LID_FILE = "/proc/acpi/button/lid/LID/state"
...
ROTATION_LOCK_STATUS = join(DIRECTORY_TEMP, "smd", "rotate.state")
...
BACKGROUND_DIRECTORY_CACHE = ".cache/smd"
...
LOCKER_STATUS_FILE = join(DIRECTORY_TEMP, "smd", "lockers.state")
LOCKER_NAMES = {
    LOCKER_LID: "Lid",
...
_init("/opt/spaceport/etc/smd/constants.json")
