...
from lib.util import read, write, stop, run, remove_file, read_json, boolean
...
class HydraVM(Storage):
            ...
            if drive["type"] == "virtio":
                ...
                build += [
                    ...
                    % (
                        ...
                        did,
                        did,
                        (1 + ide) if self.get("type") == "q35" else (ide / 2),
                        drive["index"],
                    ),
                ]
                ide += 1
    ...
    def _sleep(self, server, sleep):
        if not self._running():
            raise HydraError("VM is not currenly running!")
        ...
        if sleep and not self._sleeping:
            try:
                server.debug("HYDRA: VM(%d) Attempting to sleep vm.." % self.vmid)
                run(["/usr/bin/kill", "-STOP", "%d" % self._pid()], ignore_errors=False)
            except OSError as err:
                raise HydraError(err)
            else:
                server.debug("HYDRA: VM(%d) Was put to sleep!" % self.vmid)
                self._sleeping = True
        elif self._sleeping:
            try:
                server.debug("HYDRA: VM(%d) Attempting to wake vm.." % self.vmid)
                run(["/usr/bin/kill", "-CONT", "%d" % self._pid()], ignore_errors=False)
            except OSError as err:
                raise HydraError(err)
            else:
                server.debug("HYDRA: VM(%d) Was woken up!" % self.vmid)
                self._sleeping = False
    ...
    def _start(self, manager, server):
        ...
        if self.get("bios-version", 1) == 0:
            ...
            smbios = "type=%d" % self.get("bios-version", 1)
        build = [
            binary,
            "-runas",
            HYRDA_UID,
        ...
        build += [
            ...
            "-accel",
            "accel=%s,thread=multi" % self.get("accel", "kvm"),
            "-smp",
            "cpus=%d,cores=1" % self.get("cpu", 1),
            "-uuid",
            self.get("uuid", str(uuid4())),
            "-m",
            "size=%d" % self.get("memory", 1024),
            "-name",
            '"%s"' % self.get("name", "hydra-vm-%d" % self.vmid),
            "-pidfile",
            "%s.pid" % self._path,
            "-display",
...
class HydraServer(object):
        ...
        if exists(HYDRA_EXEC_NGINX):
            try:
                self.web_server = Popen(
                    [HYDRA_EXEC_NGINX, "-c", join(DIRECTORY_ASSETS, "nginx.conf")],
                    stderr=DEVNULL,
                    stdout=DEVNULL,
                )
            except (OSError, SubprocessError) as err:
    ...
    def hook(self, server, message):
        ...
        elif message.get("type") == "setall":
            if message.get("action") == "stop":
                for vm in self.vms.values():
                    if vm._running():
                        try:
                            vm._stop(self, server, force=False)
                        except HydraError as err:
                            server.error(
                                "HYDRA: Attempting to stop VM(%d) raised an exception!"
                                % vm.vmid,
                                err=err,
                            )
            elif message.get("action") == "sleep" or message.get("action") == "wake":
                sleep = boolean(message.get("action") == "sleep")
                for vm in self.vms.values():
                    try:
                        if sleep and not vm._sleeping:
                            vm._sleep(server, sleep)
                        elif not sleep and vm._sleeping:
                            vm._sleep(server, sleep)
                    except HydraError as err:
                        server.error(
                            "HYDRA: Attempting to sleep/wake VM(%d) raised an exception!"
                            % vm.vmid,
                            err=err,
                        )
            return {"vms": [vm._status() for vm in self.vms.values()]}
        ...
        if message.get("type") == "get":
            ...
            try:
                ...
                return vm._status()
        elif message.get("type") == "sleep" or message.get("type") == "wake":
            try:
                vm._sleep(server, boolean(message.get("type") == "sleep"))
            except HydraError as err:
                server.error(
                    "HYDRA: Attempting to sleep/wake VM(%d) raised an exception!"
                    % vm.vmid,
                ...
                return {
                    "error": 'Could not sleep/wake HydraVM "%d"! (%s)'
                    % (vm.vmid, str(err))
                }
            else:
                return vm._status()
    ...
    def hibernate(self, server, message):
        if message.get("state") == "pre":
            server.info("HYDRA: Sleeping all VMs due to sleep!")
            for vm in self.vms.values():
                try:
                    if not vm._sleeping:
        ...
        if message is not None and message.get("path"):
            try:
                ...
                if vm.vmid in self.vms and self.vms[vm.vmid]._running():
                    ...
                    vm._sleeping = self.vms[vm.vmid]._sleeping
